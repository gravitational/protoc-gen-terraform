...
`custom_types.<field>` lets you replace the generator's normal schema and copy
logic for a specific field with handwritten functions.
...
The `custom_types` value is the suffix used to find the custom functions. For
the example above, the generator expects these functions to be available:

```go
func GenSchemaStringCustom(ctx context.Context, diags *diag.Diagnostics, attr schema.Attribute) schema.Attribute
