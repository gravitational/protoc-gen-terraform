...
// Plugin is an interface used to decouple desc and plugin packages
...
func (c MessageBuildContext) GetGoType() string {
	...
	if c.desc.GoImportPath() != rootPackage {
		//c.imports.AddImport(c.desc.File().GetPackage())
		return c.imports.GoString(c.desc.File().GetPackage()+"."+name, true)
	}

	return c.imports.GoString(name, true)
}
